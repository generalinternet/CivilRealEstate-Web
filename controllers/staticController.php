...
class StaticController extends GI_Controller {
    ...
    public function actionProgressBar($attributes){
        if(GI_URLUtils::isAJAX()){
            //@todo just to make it seem longer
            sleep(1);
        }
        $total = 135;
        $itemsPerPage = 10;
        ...
        if($nextPageNumber <= $totalPages){
            $nextURL = GI_URLUtils::buildURL(array(
                'controller' => 'static',
                'action' => 'progressBar',
        ...
        $view->addSiteTitle('Example Progress Bar');
        $view->setTimeTrackerId($timeTrackerId);
        $view->setPercentage($percentage);
        $view->setProgressBarTitle('Example Progress Bar');
        $view->setProgressDesc('Currently progressing some progress.');
        if(!empty($nextURL)){
            $view->setNextURL($nextURL);
            $view->addHTML('<p>Maybe some more info about the progress that is progressing.</p>');
        } else {
            $view->setProgressForward(false);
            $view->addHTML('<p>All done here’s a link to <a href="http://generalinternet.ca/">something</a>.</p>');
        }
    ...
    public function actionSignHere($attributes){
        $form = new GI_Form('sign_here');
        
        $view = new StaticSignHereView($form);
        $view->buildForm();
        ...
        if($form->wasSubmitted() && $form->validate()){
            $user = Login::getUser();
            if($user){
                if(FileFactory::saveSignatureToModel('signature', $user, true)){
                    $success = 1;
    ...
    public function actionOpportunities($attributes){
        $typeRef = NULL;
        if(isset($attributes['type']) && $attributes['type_ref']){
            $typeRef = $attributes['type_ref'];
        }

        $contentRef = '';
        if(isset($attributes['ref'])){
            $contentRef = $attributes['ref'];
        }

        if (!empty($contentRef)) {
            $staticContentClass = 'StaticOpportunitiesDetailView';
        } else {
            $staticContentClass = 'StaticOpportunitiesView';
        }
        
        if(class_exists($staticContentClass)){
            if (!empty($contentRef)) {
                $view = new $staticContentClass($contentRef);
            } else {
                $view = new $staticContentClass($typeRef);
            }
            $attrs = array(
                'controller' => 'static',
            ...
            if (!empty($contentRef)) {
                $attrs['ref'] = $contentRef;
            }
            $breadcrumbs = array(
                array(
                    'label' => $view->getSiteTitle(),
                    'link' => GI_URLUtils::buildURL($attrs),
                ),
            );
        } elseif(file_exists('controllers/' . strtolower($attributes['action']) . 'Controller.php')){
            GI_URLUtils::redirect(array(
    ...
    public function actionAngelNetwork($attributes){
        $form = new GI_Form('contact_form');
        $form->setBotValidation(true);
        $view = new StaticAngelNetworkView($form);
        ...
        if($form->wasSubmitted() && $form->validate()){
            ...
            $email = filter_input(INPUT_POST, 'r_email');
            $phone = filter_input(INPUT_POST, 'phone');
            $bizName = filter_input(INPUT_POST, 'biz_name');
            $bizWebsite = filter_input(INPUT_POST, 'biz_website');
            $bizDesc = filter_input(INPUT_POST, 'biz_desc');
            ...
            if(!empty($bizName)){
                $emailView->addHTML('<br/>Business Name: <b>' . $bizName . '</b>');
            }
            if(!empty($bizWebsite)){
                $emailView->addHTML('<br/>Business Website: <b>' . $bizWebsite . '</b>');
            }
            $emailView->closeParagraph()
                    ->addParagraph(nl2br($bizDesc))
                    ->closeBlock();
            ...
            $giEmail->addTo(SITE_EMAIL, EMAIL_TITLE)
                    ->addCC('david.kolby@generalinternet.ca', 'David Kolby')
                    ->setFrom(ProjectConfig::getServerEmailAddr(), ProjectConfig::getServerEmailName())
                    ->setSubject('Angel Network Contact Form Message')
                    ->useEmailView($emailView);
            if($giEmail->send()){
                $newAttributes = $attributes;
                $newAttributes['sent'] = 1;
