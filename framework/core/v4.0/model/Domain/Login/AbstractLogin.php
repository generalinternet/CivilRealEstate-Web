...
abstract class AbstractLogin extends GI_Model {
    ...
    public static function setUserLang() {
        $userLang = SessionService::getValue('user_lang');
        if (empty($userLang)) {
            ...
            if ($userLang) {
                ...
                SessionService::setValue('user_lang', $userLang);
            } else {
                require_once('config/language/lang_english.php');
                SessionService::setValue('user_lang', 'english');
            }
        } else {
            $filename = 'lang_' . $userLang . '.php';
            require_once('config/language/' . $filename);
    ...
    public static function getCurrentInterfacePerspectiveRef() {
        $user = static::getUser();
        if (!empty($user)) {
            $currentInterfacePerspective = $user->getCurrentInterfacePerspective();
            if (!empty($currentInterfacePerspective)) {
                return $currentInterfacePerspective->getProperty('ref');
            }
        }
        return NULL;
    }
    
    public static function interfacePerspectiveRefEquals($ref) {
        $currentInterfacePerspectiveRef = static::getCurrentInterfacePerspectiveRef();
        if (!empty($currentInterfacePerspectiveRef) && $currentInterfacePerspectiveRef === $ref) {
            return true;
        }
        return false;
    }
