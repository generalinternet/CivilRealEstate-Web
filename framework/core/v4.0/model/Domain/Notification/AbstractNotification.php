...
//            return false;
//        }
        return NotificationService::notifyBySocketId($socketId);
    }
    
    public static function qbConnected($socketUserId){
        $client = new Client(new Version1X(ProjectConfig::getSocketServerURLWithPort()));
        try {
            $client->initialize();
            $client->emit('qb_connect', array('socketUserId' => $socketUserId));
            $client->close();
            return true;
        } catch (Exception $ex) {
            return false;
    ...
    public static function markNotificationViewed() {
        ...
        $url= ProjectConfig::getHTMLProtocol() . '://' . $server . $path;
            ...
            $notifications = NotificationFactory::search()
                    ...
                    ->filterGroup()
                        ...
                        ->filterGroup()
                            ->filterLike('url', '%' . $path)
                            ->andIf()
                            ->filterLike('url', ProjectConfig::getHTMLProtocol() . '://' . $server . '%')
                        ->closeGroup()
                    ->closeGroup()
                    ->andIf()
                    ->filter('viewed', 0)
    ...
    /**
     * @deprecated - Remove in V5 - use Events/Notification Service instead
    ...
    public static function notifyUser(AbstractUser $user, $subject, $attributes = NULL, $msg = NULL, $type = NULL) {
        $currentUserId = Login::getUserId(true);
        ...
        if ((int) $currentUserId != (int) $userId) {
            ...
            $notification->setProperty('from_id', Login::getUserId(true));
            ...
            if ($notification->save()) {
                ...
                foreach ($socketUsersToNotify as $socketUserId) {
                    NotificationService::notifyBySocketUserId($socketUserId);
                }
                return true;
            }
        }
    ...
    public function setToUser(AbstractUser $user) {
        $this->toUser = $user;
        $this->setProperty('to_id', $user->getId());
    }
    ...
    public function save() {
        ...
        if(empty($this->getProperty('to_id'))){
            return true;
        }
        if(empty($this->getProperty('from_id'))){
            $this->setProperty('from_id', $userId);
        }
        return parent::save();
    }
