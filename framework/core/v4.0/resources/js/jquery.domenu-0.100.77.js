 ...
 * @version 0.100.77
 * MIT license
 * Edited by General Internet
 */

;
(function ($, window, document, undefined) {
  var hasTouch = 'ontouchstart' in document;
  var isOnAnimating = false;
  ...
  var defaults = {
    ...
    wrapperClass:           'dd-block-list',
    subListClass:           'dd-row-list',
    event:                  {
      onToJson:           [],
      onParseJson:        [],
      onSaveEditBoxInput: [],
  ...
  Plugin.prototype = {
    ...
    addNewListItemListener:           function (addBtn, parent) {
      ...
      addBtn.on('click', function (e) {
        ...
        var blueprintClass = $(this).data('blueprint');
        if (blueprintClass != undefined) {
            var item = _this.createNewListItem(null, blueprintClass);
        } else {
            var item = _this.createNewListItem();
        }
     ...
     * @returns {*}
     */
    createNewListItem:                function (data, blueprintClass) {
      var _this           = this,
          el              = this.$instance,
          opt             = this.options;
      ...
      if (blueprintClass != undefined) {
          var blueprint       = el.find(blueprintClass.dot()).first().clone();
      } else {
          var blueprint       = el.find(opt.itemBlueprintClass.dot()).first().clone();
      }
      if (blueprint.length > 0) {
            var inputCollection = blueprint.find(opt.editBoxClass.dot()).find(opt.inputSelector);
            ...
            if (blueprint.find('.item-edit').length > 0){
    ...
    reset: function () {
      ...
      this.$pointEl = null;
      this.dragStartRootEl = null; //Added it because dragRootEl keeps changing once the root change while dragging
                                   //dragStartRootEl is only drag start root element.
    },
    ...
    setParent:                          function ($item, force) {
      ...
      if ($item.find(this.options.subListClass.dot()).length || force) {
        // LI STRUCTURE
      ...
      //expand/collaps buttons
      if($item.find(opt.subListClass.dot()).children(opt.itemNodeName).length == 0){
          $item.children('[data-action="collapse"]').hide();
          $item.children('[data-action="expand"]').hide();
      } else {
          if ($item.hasClass(opt.collapsedClass)) {
            $item.children('[data-action="collapse"]').hide();
            $item.children('[data-action="expand"]').show();
        } else {
            $item.children('[data-action="collapse"]').show();
            $item.children('[data-action="expand"]').hide();
        }
    ...
    dragStart:                          function (e) {
      ...
      //$(this.options.listClass.dot()).addClass('dragging');
      ...
      this.dragRootEl = $(dragItem).closest(opt.rootClass.dot());
      this.dragStartRootEl = $(dragItem).closest(opt.rootClass.dot());

        //Set start position
        var startHPos = parseInt($(dragItem).parents(opt.listClass.dot()).length) - 1;
        var startVPos = $(this.dragStartRootEl).find(this.options.itemClass.dot()).index(dragItem);
        $(dragItem).data('start_h_pos', startHPos);
        $(dragItem).data('start_v_pos', startVPos);
        //Set start position: end
      ...
      .addClass(this.options.listClass + ' ' + this.options.dragClass + ' dragging');
    ...
    dragStop:                           function (e) {
      ...
      this.$placeholder.replaceWith(el);
        ...
        //Compare with the start position to defined the position has changed
        $rootListOfPointingElement = $(el).closest(this.options.rootClass.dot());
        var hasPositionChanged = $(this.dragStartRootEl).data('domenu-id') !== $rootListOfPointingElement.data('domenu-id');
        if (!hasPositionChanged) {
            var startHPos = $(el).data('start_h_pos');
            var startVPos = $(el).data('start_v_pos');
            var stopHPos = parseInt($(el).parents(this.options.listClass.dot()).length) - 1;
            var stopVPos = $(this.dragStartRootEl).find(this.options.itemClass.dot()).index(el);
           
            if (startHPos !== stopHPos || startVPos !== stopVPos) {
...
//        });
//      }
      $(this.options.listClass.dot()).removeClass('dragging');
    },
    ...
    dragMove: function (e) {
      ...
      // wrapper class offset
      var wrapperEl = $(this.options.wrapperClass.dot());
      var wrapperX = wrapperEl.offset().left;
      var wrapperY = wrapperEl.offset().top;
      var wrapperMaxX = wrapperX + wrapperEl.width();
      var wrapperMaxY = wrapperY + wrapperEl.height();
      var screenTop = $(document).scrollTop();
      var scrollBottom = $(document).scrollTop() + $(window).height();
      // If mouse is out of wrapper block, stop moving
      if (mouse.nowX < wrapperX || mouse.nowX > wrapperMaxX) {
          return;
      }
      if (mouse.nowY < wrapperY || mouse.nowY > wrapperMaxY) {
          return;
      }
      var yScrollStep = 200;
      if (mouse.nowY < screenTop || mouse.nowY > scrollBottom) {
          
            if (mouse.nowY < screenTop) {
                var yScrollTarget = screenTop - yScrollStep;
                if (yScrollTarget < wrapperY - 50) {
                    yScrollTarget = wrapperY - 50;
                }
            } else if (mouse.nowY > scrollBottom) {
                var yScrollTarget = scrollBottom + yScrollStep - $(window).height();
                if (yScrollTarget > wrapperMaxY + 50) {
                    yScrollTarget = wrapperMaxY + 50;
                }
            }
      ...
      if (Math.abs(mouse.distXtotal) >= opt.threshold) {
        ...
        if (mouse.distXtotal > 0 && mouse.dirX === 1 && $precedingSiblingItem.length > 0 && !$precedingSiblingItem.hasClass(opt.collapsedClass)
                &&  !$precedingSiblingItem.hasClass(opt.noSubItemClass)) {
console.log('To right');             
          ...
          if ($parent.length) {
            ...
            // Commented out because it causes the placeholder disappear when the list has only 1 element and move to the lett
//            if ($parent.children(opt.itemClass.dot()).length === 0) {
//                this.unsetEmptyParent($parent);
//            }
            setTotalDistance(0, 0);
          }
        }
      ...
      if (mouse.isMovingOnXAxis === false && Math.abs(mouse.distYtotal) >= 5) {
        ...
        var rootEl = this.$placeholder.closest(this.options.rootClass.dot());  
        if (rootEl.find(this.options.itemClass.dot()).length === 0) {
            rootEl.find(this.options.listClass.dot()).first().addClass('empty');
        }
        ...
        if ($placeholderParent.children().length === 0) {
            //Add the condition because if the first list is deleted, we can't drag it back
            if ($placeholderParent.parents(opt.listClass.dot()).length > 0) {
    ...
    updateBtns:                          function () {
      var opt = this.options;
      // forEach itemNodeName=li element
      $.each(this.$instance.find(opt.itemNodeName), function (k, el) {
        //expand/collaps buttons
        if($(el).find(opt.subListClass.dot()).children(opt.itemNodeName).length == 0){
            $(el).children('[data-action="collapse"]').hide();
            $(el).children('[data-action="expand"]').hide();
        } else {
            if ($(el).hasClass(opt.collapsedClass)) {
              $(el).children('[data-action="collapse"]').hide();
              $(el).children('[data-action="expand"]').show();
          } else {
              $(el).children('[data-action="collapse"]').show();
              $(el).children('[data-action="expand"]').hide();
          }
        }
      });
  ...
  $.fn.domenu = function (params) {
    var lists   = this.first(),
        domenu  = $(this);
    var plugin  = domenu.data("domenu") || new Plugin(this, params);
    //var plugin  = new Plugin(this, params);
    var pPlugin = new PublicPlugin(plugin, lists);
    ...
    if (!domenu.data("domenu-id")) {
        //In case of defining by a class name, object is a array
        //So set id for all elements with the class name
//        var pseudoRandomNumericKey = Math.random().toString().replace(/\D/g, '');
//        domenu.data('domenu-id', pseudoRandomNumericKey);
        for (var i=0; i<domenu.length; i++) {
            var pseudoRandomNumericKey = Math.random().toString().replace(/\D/g, '');
            $(domenu[i]).data('domenu-id', pseudoRandomNumericKey);
        }
    }
    //update expand/contract icons
    plugin.updateBtns();
