<?php
/**
 * Description of AbstractFormStepView
 *
 * @author General Internet
 * @copyright  2017 General Internet
 * @version    4.0.0
 */
abstract class AbstractFormStepView extends MainWindowView {
    
    public static $FORM_HANDLER_FUNCTION_NAME_FIELD_NAME = 'form_handler_function_name';
    protected $viewBodyId = 'form_step_view_wrap';
    ...
    protected $formBodyWrapClass = array();
    protected $formBodyClass = array();
    protected $btnClass = array();
    protected $sideStepNav = true;
    protected $showStepNav = true;
    protected $showStepTitle = true;
    ...
    public function addFormBodyWrapClass($class) {
        if (!in_array($class, $this->formBodyWrapClass)) {
            $this->formBodyWrapClass[] = $class;
        }
        return $this;
    }
    
    protected function getFormBodyWrapClass(){
        $formBodyWrapClass = '';
        if (!empty($this->formBodyWrapClass)) {
            $formBodyWrapClass .= implode(' ', $this->formBodyWrapClass);
        }
        return $formBodyWrapClass;
    }
    ...
    public function setShowStepNav($showStepNav) {
        $this->showStepNav = $showStepNav;
        return $this;
    }
    
    public function setShowStepTitle($showStepTitle) {
        $this->showStepTitle = $showStepTitle;
        return $this;
    }
    ...
    protected function openFormWrap($classNames = ''){
        ...
        if ($nextStep === -1) {
            $classNames .= ' last_step';
        }
        $formBodyWrapClass = $this->getFormBodyWrapClass();
        if (!empty($formBodyWrapClass)) {
          $classNames .= ' '.$formBodyWrapClass;
        }
        
        $this->form->addHTML('<div class="form_body_wrap step_' .$this->curStep. ' '. $classNames . '" data-step="'.$this->curStep.'" data-total-step="'.$this->totalSteps.'">');
    }
    ...
    */
    ...
    public function buildForm() {
        if (!$this->formBuilt) {
            ...
            $this->openFormWrap();
            $this->form->addFormClass('step_form');
            if ($this->sideStepNav) {
                //Left step nav
