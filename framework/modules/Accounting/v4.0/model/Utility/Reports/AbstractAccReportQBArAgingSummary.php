<?php
/**
 * Description of AbstractAccReportQBArAgingSummary
 *
...
abstract class AbstractAccReportQBArAgingSummary extends AbstractAccReportQB {
    ...
    protected function getQBReportName() {
        return ReportName::AGEDRECEIVABLES;
    }
    
    public function getDescription() {
        return 'Shows unpaid invoices for the current period and for the last 30, 60 and 90+ days so you can see how long they’ve been open (outstanding).';
    }
    ...
    public function isViewable() {
        if ($this->overridePermissionCheck || Permission::verifyByRef('view_ar_aging_summary_report')) {
            return true;
        }
        return false;
    }
    ...
    public function getCurrentTotal($formatForDisplay = false) {
        ...
        if (is_null($this->currentTotal) && !$this->reportBuilt) {
            $cachedValue = $this->getValueFromCache('ar_current_total');
        ...
        if ($setCache) {
            $this->setValueInCache('ar_current_total', $value);
        }
        if ($formatForDisplay) {
    ...
    public function getOneToThirtyTotal($formatForDisplay = false) {
        ...
        if (is_null($this->oneToThirtyTotal) && !$this->reportBuilt) {
            $cachedValue = $this->getValueFromCache('ar_one_to_thirty_total');
            if (!empty($cachedValue)) {
        ...
        if ($setCache) {
            $this->setValueInCache('ar_one_to_thirty_total', $value);
        }
        if ($formatForDisplay) {
            return '$' . GI_StringUtils::formatMoney($value);
        }
    ...
    public function getThirtyOneToSixtyTotal($formatForDisplay = false) {
        ...
        if (is_null($this->thirtyOneToSixtyTotal) && !$this->reportBuilt) {
            $cachedValue = $this->getValueFromCache('ar_thirty_one_to_sixty_total');
            if (!empty($cachedValue)) {
        ...
        if ($setCache) {
            $this->setValueInCache('ar_thirty_one_to_sixty_total', $value);
        }
        if ($formatForDisplay) {
            return '$' . GI_StringUtils::formatMoney($value);
        }
    ...
    public function getSixtyOneToNinetyTotal($formatForDisplay = false) {
        ...
        if (is_null($this->sixtyOneToNinetyTotal) && !$this->reportBuilt) {
            $cachedValue = $this->getValueFromCache('ar_sixty_one_to_ninety_total');
            if (!empty($cachedValue)) {
        ...
        if ($setCache) {
            $this->setValueInCache('ar_sixty_one_to_ninety_total', $value);
        }
        if ($formatForDisplay) {
            return '$' . GI_StringUtils::formatMoney($value);
        }
    ...
    public function getNinetyOneAndOverTotal($formatForDisplay = false) {
        ...
        if (is_null($this->ninetyOneAndOverTotal) && !$this->reportBuilt) {
            $cachedValue = $this->getValueFromCache('ar_ninety_one_and_over_total');
            if (!empty($cachedValue)) {
        ...
        if ($setCache) {
            $this->setValueInCache('ar_ninety_one_and_over_total', $value);
        }
        if ($formatForDisplay) {
            return '$' . GI_StringUtils::formatMoney($value);
        }
    ...
    public function getTotal($formatForDisplay = false) {
        $setCache = false;
        if (is_null($this->arTotal) && !$this->reportBuilt) {
            $cachedValue = $this->getValueFromCache('ar_total');
            if (!empty($cachedValue)) {
                $this->arTotal = $cachedValue;
            } else {
                $this->buildReport();
                $setCache = true;
            }
        }
        if (is_null($this->arTotal)) {
            return NULL;
        }
        $value = $this->arTotal;
        if ($setCache) {
            $this->setValueInCache('ar_total', $value);
        }
        if ($formatForDisplay) {
            return '$' . GI_StringUtils::formatMoney($value);
        }
    ...
    protected function parseTotalsFromTotalsRowData($totalsRow) {
        ...
        if (!is_null($currentTotal)) {
            ...
            $this->setValueInCache('ar_current_total', $currentTotal);
        }
        if (!is_null($oneToThirty)) {
            $this->oneToThirtyTotal = $oneToThirty;
            $this->setValueInCache('ar_one_to_thirty_total', $oneToThirty);
        }
        if (!is_null($thirtyOneToSixty)) {
            $this->thirtyOneToSixtyTotal = $thirtyOneToSixty;
            $this->setValueInCache('ar_thirty_one_to_sixty_total', $thirtyOneToSixty);
        }
        if (!is_null($sixtyOneToNinety)) {
            $this->sixtyOneToNinetyTotal = $sixtyOneToNinety;
            $this->setValueInCache('ar_sixty_one_to_ninety_total', $sixtyOneToNinety);
        }
        if (!is_null($ninetyOneAndOver)) {
            $this->ninetyOneAndOverTotal = $ninetyOneAndOver;
            $this->setValueInCache('ar_ninety_one_and_over', $ninetyOneAndOver);
        }
        if (!is_null($total)) {
            $this->arTotal = $total;
            $this->setValueInCache('ar_total', $total);
        }
    }
