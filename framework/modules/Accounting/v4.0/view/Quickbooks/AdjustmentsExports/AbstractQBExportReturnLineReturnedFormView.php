...
abstract class AbstractQBExportReturnLineReturnedFormView extends AbstractExportAdjustmentsToQBFormView {
    
    /** @var AbstractOrderReturnLineReturned */
    protected $model;

    public function __construct(\GI_Form $form, AbstractOrderReturnLineReturned $model, $values) {
    ...
    protected function addExportableModelInfoSection() {
        ...
        $this->form->addHTML('<h2 class="form_section_title">Returned Item(s)</h2>');
    ...
    protected function buildFormBody() {
        $this->form->addHTML('<div class="form_section">');
        $this->form->addHTML('<h2 class="form_section_title">Returned Stock Expenses</h2>');
        ...
        if (!empty($values)) {
            foreach ($values as $currencyRef=>$currencyValues) {
                ...
                $this->form->addHTML('<div class="flex_row flex_head">')
                        ...
                        ->addHTML('<div class="flex_col med">Debit Account</div>')
                ...
                foreach ($currencyValues as $qbInvAssetAccountQbId => $assetValues) {
                    ...
                    foreach ($assetValues as $poLineTypeRef => $valueArray) {
                        if (!empty($valueArray)) {
                            ...
                            $debitAccountQBId = $valueArray['debit_acct_qb_id'];
                            $invAssetAccount = QBAccountFactory::getModelByQBId($debitAccountQBId);
                            ...
                            $this->form->addHTML('</div>')
                                    ->addHTML('<div class="flex_col med">');
                            $this->form->addField('debit_acct_qb_id_' . $seqNum, 'hidden', array(
                                'value' => $debitAccountQBId,
                            ));
