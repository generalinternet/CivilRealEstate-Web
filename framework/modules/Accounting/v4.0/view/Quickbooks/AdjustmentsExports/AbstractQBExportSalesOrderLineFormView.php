...
abstract class AbstractQBExportSalesOrderLineFormView extends AbstractExportAdjustmentsToQBFormView {
    
    /** @var AbstractOrderLineSales */
    protected $model;

    public function __construct(\GI_Form $form, AbstractOrderLineSales $model, $values) {
    ...
    protected function addExportableModelInfoSection() {
        ...
        $qty = $this->model->getQty(true, true);
        ...
        $this->form->addHTML('<h2 class="form_section_title">Sales Order Item</h2>');
    ...
    protected function buildFormBody() {
        $this->form->addHTML('<div class="form_section">');
        $this->form->addHTML('<h2 class="form_section_title">Sold Stock Expenses</h2>');
        
        $seqNum = 0;
       // $values = $this->model->getNetExpenseCogsSumsArray();
        $values = $this->values;
        if (!empty($values)) {
                ...
                foreach ($currencyValues as $qbInvAssetAccountQbId => $assetValues) {
                    ...
                    foreach ($assetValues as $poLineTypeRef => $value) {
                        if (!empty($value)) {
                            ...
                            $this->form->addHTML('<input name="qb_adj_sol_seq_nums[]" value="' . $seqNum . '" type="hidden" class="seq_count"/>');
    ...
    protected function addDebitAccountField($seqNum, $poLineTypeRef) {
        $samplePOLine = OrderLineFactory::buildNewModel($poLineTypeRef);
        $invItem = $this->model->getInventoryItem();
        if (!empty($invItem)) {
            $samplePOLine->setProperty('order_line_purchase.inv_item_id', $invItem->getId());
        }
        $value = $samplePOLine->getQBCOGSAccountQBId();
