 ...
 * Customized by General Internet
 * (c) 2017 Adam Shaw, Hak Choi
 */
...
function compensateScroll(rowEls, scrollbarWidths) {
	if (scrollbarWidths.left) {
		rowEls.css({
			'border-left-width': 1,
			//'margin-left': scrollbarWidths.left - 1 Hacked by Hak : adjust margin to match borders inbetween header and body
                        'margin-left': scrollbarWidths.left
		});
	}
	if (scrollbarWidths.right) {
		rowEls.css({
			'border-right-width': 1,
			//'margin-right': scrollbarWidths.right - 1 Hacked by Hak : adjust margin to match borders inbetween header and body
                        'margin-right': scrollbarWidths.right
		});
	}
}
...
Grid.mixin({
	...
	compareEventSegs: function(seg1, seg2) {
                /* Hacked by Hak : if eventOrder is set explicitly order by eventOrderSpecs first */
                /*
			...
			compareByFieldSpecs(seg1.event, seg2.event, this.view.eventOrderSpecs);
                */
                if (this.view.eventOrderSpecs.length == 1 && this.view.eventOrderSpecs[0].field === 'title' ) {
                    // Default event order (title)
                    return  seg1.eventStartMS - seg2.eventStartMS || // earlier events go first
                            seg2.eventDurationMS - seg1.eventDurationMS || // tie? longer events go first
                            seg2.event.allDay - seg1.event.allDay || // tie? put all-day events first (booleans cast to 0/1)
                            compareByFieldSpecs(seg1.event, seg2.event, this.view.eventOrderSpecs);
                } else {
                    // event order is set explicitly : order by the event order field first
                    return  compareByFieldSpecs(seg1.event, seg2.event, this.view.eventOrderSpecs) || //order by the event order field
...
var DayTableMixin = FC.DayTableMixin = {
	...
	breakOnWeeks: false, // should create a new row for each week?
        breakOn2Weeks: false, //Hacked by Hak: break on every 2 weeks(month2wks viewer)
	...
	updateDayTable: function() {
		...
		if (this.breakOnWeeks) {
			...
			firstDay = dayDates[0].day();
                        var weekCnt = 0;// Hacked by Hak 
			for (daysPerRow = 1; daysPerRow < dayDates.length; daysPerRow++) {
				if (dayDates[daysPerRow].day() == firstDay) {
                                        /* Hacked by Hak : for the month2wks viewer, skip break if the first week */
                                        if (this.breakOn2Weeks) {
                                               if (weekCnt == 1) {
                                                   // Break at the second week
                                                    break;
                                                } else {
                                                    weekCnt++;
                                                }
	...
	renderHeadDateCellHtml: function(date, colspan, otherAttrs) {
		...
		if (this.rowCnt === 1) {
			...
			classNames.push('fc-' + dayIDs[date.day()]); // only add the day-of-week class
                ...
                /* Hacked by Hak : change header format by view types */
                var date_html = '';
                if (view.name == 'month2wks') {
                    date_html = htmlEscape(date.format(this.colHeadFormat).substr(0,1)); // show the initial letter on month2wks view
                } else if (view.name == 'week' || view.name == 'agendaWeek' ) {
                    date_html = date.format("ddd") 
                           + '<span class="desktop-visible fc-day-text"> ' + date.format("MMMM D") + '</span>'
                          //  + '<span class="mobile-visible fc-day-text">' + date.format("MMM D") + '</span>';
                } else {
                    date_html = htmlEscape(date.format(this.colHeadFormat));
                }
                /* Hacked by Hak - end */
		return '' +
            '<th class="' + classNames.join(' ') + '"' +
				...
				view.buildGotoAnchorHtml(
					{ date: date, forceOff: this.rowCnt > 1 || this.colCnt === 1 },
                                        /* Hacked by Hak : show the initial letter on month2wks view */
					//htmlEscape(date.format(this.colHeadFormat)) // inner HTML
                                        date_html// inner HTML
                                        /* Hacked by Hak - end */
				) +
			'</th>';
	},
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderNumberCellHtml: function(date) {
		...
		if (this.view.dayNumbersVisible) {
                        /* Hacked by Hak: show month name at the first day of week on the month2wks view*/
                        var date_html = '';
                        if (this.view.name == 'month2wks' && (date.day() == date._locale.firstDayOfWeek())) {
                                date_html += '<span>' + date.format('MMM') + '</span> ' + date.date();
                        } else {
                                date_html += date.date();
                        }
                        /* Hacked by Hak - end */
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		classes.unshift('fc-day-grid-event', 'fc-h-event');
                /* Hacked by Hak : add a event id to find the same event segment in other cells */
                if (event.id) {
                    classes.push('fc-event-id-' + event.id);
                }
                /* Hacked by Hak - end */
                /* Hacked by Hak : add status */
                if (event.status) {
                    classes.push('fc-event-status-' + event.status);
                }
                /* Hacked by Hak - end */
		...
		titleHtml =
			'<span class="fc-title">' +
                                /*Hacked by Hak : Change event height on the month view */
                                //(htmlEscape(event.title || '') || '&nbsp;') + // we always want one line of height
                                (event.title ? (' <strong>'+event.title+'</strong>') : '') +
                                ((event.resources && event.resources.length>0) ?
                                        '<span class="fc-resources">' +
                                                getInitials(event.resources).join(" & ") +
                                        '</span>' :
                                        ''
                                        ) +
                                /*Hacked by Hak - ended */
		...
		while (slotTime < this.maxTime) {
			...
			axisHtml =
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
                                        /** Hacked by Hak: show 30minutes too **/
                                        /*
					(isLabeled ?
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
					/*Hacked by Hak : Remove time and title on the week view*/
                                        /*
                                        (timeText ?
						'<div class="fc-time"' +
					...
					(event.title ?
						...
						''
                                        ...
                                        */
                                        /*Hacked by Hak - ended*/
                                        /*Hacked by Hak : Add event's resources */
                                        ((event.resources && event.resources.length>0)?
						'<div class="fc-resources">' +
							getInitials(event.resources).join("<br>") +
						'</div>' :
						''
						) +
                                        /*Hacked by Hak - ended */
	...
	generateFgSegHorizontalCss: function(seg) {
		...
		if (this.isRTL) {
			...
			right = backwardCoord;
                        ...
                        /* Hacked by Hak : to void overlapping between events cells even thought the setting is non-overlapping*/
			//left = backwardCoord;
                        if (backwardCoord == 0){
                            left = 0;
                        } else {
                            left = backwardCoord + 0.03; // Add a gap 0.02% from the second event
                        }
                        /* Hacked by Hak - end */
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				if (groupChildren.length > 1) {
					groupEl = $('<div/>');
					if (isOnlyButtons) {
						//groupEl.addClass('fc-month-button'); Hacked by Hak: The month view button class is also "fc-month-button"
                                                groupEl.addClass('fc-view-buttons');
					}
...
function Calendar_constructor(element, overrides) {
	...
	function initialRender() {
		...
		renderHeader();
		renderFooter();
                /*Hacked by Hak: check screen size before render. 
                 * If the screen width is mobile size, set mobile options*/
                var window_width = window.innerWidth;
                if (window_width < t.options.mobileWidth) {
                    t.options.defaultView = t.options.mobileDefaultView;
                    t.options.contentHeight = t.options.mobileContentHeight;
                }
                /*Hacked by Hak - end*/
...
Calendar.defaults = {
	...
	// display
	defaultView: 'month',
        defaultMobileView: 'month', //Hacked by Hak: set mobile default view
        mobileWidth: 960, //Hacked by Hak: set mobile screen width 
	aspectRatio: 1.35,
	header: {
		left: 'title',
		center: '',
...
function EventManager() { // assumed to be a calendar
	...
	function filterEventsWithinRange(events) {
		...
		for (i = 0; i < events.length; i++) {
			...
			if (    
                                /* Hacked by Hak : 
                                    Bug - the events on the end date don't show
                                    Fix bug - rangeEnd should be added by 1 day 
                                                because the rangeEnd's time is 00:00 and the condition is less then rangeEnd's time
                                   Example - Dayview's rangeEnd, rangeStart is the same date so there is no event shown
                                */
				//event.start.clone().stripZone() < rangeEnd &&
                                event.start.clone().stripZone() < (rangeEnd + (24*60*60*1000)) &&
                                /* Hacked by Hak - end */
	...
	function buildEventFromInput(input, source) {
		...
		if (input.className) {
			...
			out.className = [];
                ...
                //Hacked by Hak - add event's resources - string(space delimiter)/array
                if (out.resources) {
...
/* Hacked by Hak : Create Month2WksView
 * A month view with day cells running in rows (one-per-2week) and columns
----------------------------------------------------------------------------------------------------------------------*/
var Month2WksView = BasicView.extend({
	// Produces information about what range to display
	computeRange: function(date) {
		var range = BasicView.prototype.computeRange.call(this, date); // get value from super-method
		var rowCnt;
                ...
                // ensure 8 weeks 
                rowCnt = Math.ceil(range.end.diff(range.start, 'weeks', true)); // could be partial weeks due to hiddenDays
                range.end.add(this.options.durantionweeks - rowCnt, 'weeks');
	...
	setRange: function(range) {
		...
		this.dayGrid.breakOnWeeks = true;
                this.dayGrid.breakOn2Weeks = true;
		this.dayGrid.setRange(range);
	},
	// Overrides the default BasicView behavior to have special multi-week auto-height logic
	setGridHeight: function(height, isAuto) {

		// if auto, make the height of each row the height that it would be if there were 4 rows
		if (isAuto) {
                        // Divided by 4 rows(Period weeks/2)
                        height *= this.rowCnt / this.options.durantionweeks / 2;
		}
...
fcViews.month2wks = {
	'class': Month2WksView,
        duration: { weeks: 8 } // important for prev/next
};
;;
/* Hacked by Hak -end */
...
var ListViewGrid = Grid.extend({
	...
	spanToSegs: function(span) {
		...
		var segs = [];
                /* Hacked by Hak : 
                    Bug - the events on the end date don't show
                    Fix bug - view.end should be added by 1 day 
                              because the view.end's time is 00:00 and the condition is less then view.end's time
                 */
		//while (dayStart < view.end) {
                while (dayStart < view.end + (24*60*60*1000)) {
...
/** Hacked by Hak : Get intials from resource **/
function getInitials(arr) {
    var arr_initial = new Array();
    var arr_initial_letter = new Array();
    var arr_names = new Array();
    if (arr !== undefined) {
        for (i = 0; i < arr.length; i++) {
            //Split a full name by space 
            arr_names = arr[i].split(/\s+/) 
            for (j = 0; j < arr_names.length; j++) {
                //Get the first letter by uppercase
                arr_initial_letter[j] = arr_names[j].charAt(0).toUpperCase();
            }
            //Join the each initial letters
            arr_initial[i] = arr_initial_letter.join('');
        }
    }
    return arr_initial;
}
